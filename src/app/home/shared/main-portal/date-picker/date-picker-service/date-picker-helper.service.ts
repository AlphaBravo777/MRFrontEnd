...
export class DatePickerHelperService {
    ...
    changeYearWeekDayIntoLongDate(datePackage: IDate) { // Takes a week and year number and turns it into a date
        try {
            ...
            const date = this.addDaysToDate(ISOweekStart, datePackage.weekDay - 1);
    ...
    addDaysToDate(longDate: Date, days: number) {
        const result = new Date(longDate);
    ...
    convertLongDateIntoYearWeekWeekDay(datePackage: IDate) {  // This is of the new generation date functions
        let longDate: any = datePackage.longDate;
        longDate = new Date(Date.UTC(longDate.getFullYear(), longDate.getMonth(), longDate.getDate())); // Do not modify original longdate
